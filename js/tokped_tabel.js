...
fetch("tokopedia/laptop_gaming_tokopedia.json")
    ...
    .then(function(products) {
        ...
        document.getElementById('sortHarga').addEventListener('click', () => sortProducts('harga'));
        document.getElementById('sortRating').addEventListener('click', () => sortProducts('rating'));
        document.getElementById('sortTerjual').addEventListener('click', () => sortProducts('terjual'));
        ...
        function sortProducts(column) {
            ...
            if (typeof a[column] === 'string') {
                return a[column].localeCompare(b[column], 'id', { numeric: true }) * (direction === 'asc' ? 1 : -1);
            } else {
                return (a[column] - b[column]) * (direction === 'asc' ? 1 : -1);
            }
            });
        ...
        // Tambahkan variabel untuk menyimpan kolom dan arah sorting saat ini
        ...
        function displayCurrentPage(sortedProducts) {
        ...
        function search_laptop() {
            ...
            displayPaginatedProducts(filteredProducts); // Menampilkan hasil pencarian dengan pemfilteran halaman
        ...
        function displayPaginatedProducts(filteredProducts) {
            const totalPages = Math.ceil(filteredProducts.length / rowsPerPage); // Hitung total halaman berdasarkan jumlah produk dan jumlah baris per halaman
            const startIndex = (currentPage - 1) * rowsPerPage;
            const endIndex = startIndex + rowsPerPage;
            const currentProducts = filteredProducts.slice(startIndex, endIndex);
            displayProducts(currentProducts);
            updatePagination(totalPages, currentPage); // Perbarui tampilan navigasi berdasarkan total halaman dan halaman saat ini
        }
